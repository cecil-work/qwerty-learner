...
const Speed: React.FC<SpeedProps> = ({ correctCount, inputCount, isStart, fullWidth }) => {
  ...
  return (
    <div
      ...
      } flex bg-white dark:bg-gray-800 transition-colors duration-300 mt-auto rounded-large card p-4 py-10 opacity-45`}
    >
      <InfoBox info={`${minutesStirng}:${secondsStirng}`} description="用时" />
      <InfoBox info={inputCount + ''} description="输入数" />
      <InfoBox info={speed + ''} description="速度" />
      <InfoBox info={correctCount + ''} description="正确" />
